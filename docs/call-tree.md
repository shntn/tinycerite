...
│  │  │  ├─ is_keyword                             — 文字列がキーワード(var/bit/clock/module/port/input/output/testbench/initial/step)か判定する
...
- `parse_module_def`内の`parse_decl`/`parse_stmt`は、`parse_testbench_def`内で呼ばれるものと全く同じ関数（`module_def`本体の並行部分も`testbench_def`の並行部分も同じ意味論のため）。図では重複を避けて`parse_testbench_def`側だけ展開している。

...
│              │     └─ parse_expression_factor      — 連鎖の最下段。ident/number/bitvec_literal/括弧を解決する
│              │        ├─ is_keyword                — 文字列がキーワード(var/bit)か判定する
...
│  │  ├─ NetlistBuilder::build_expr (再帰)           — 解決済み式からノードを再帰的に構築する(BinOp/UnaryOp/Ternaryの結果幅も決定。BitVecLiteralは幅切り詰めてmake_constへ)

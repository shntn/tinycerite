...
│  │  │  ├─ is_keyword                             — 文字列がキーワード(var/bit/clock/module/port/input/output/testbench/initial/step/if/elif/else)か判定する
...
│  │  │  │     └─ parse_signal_type                — signal_typeルールのペアをSignalTypeに変換する(下記に展開)
│  │  │  ├─ parse_decl                             — testbench_def内のparse_declと同一(下記に展開)
│  │  │  ├─ parse_stmt                             — testbench_def内のparse_stmtと同一(下記に展開)
│  │  │  └─ parse_if_stmt                          — testbench_def内のparse_if_stmtと同一(下記に展開)
│  │  └─ parse_testbench_def                       — testbenchルールのペアをTestbenchDefに変換する(キーワード名を拒否)
│  │     ├─ is_keyword                             — 文字列がキーワードか判定する
│  │     ├─ parse_decl                             — declルールのペアをDeclに変換する(キーワード名を拒否)
│  │     │  ├─ is_keyword                          — 文字列がキーワードか判定する
│  │     │  └─ parse_signal_type                   — parse_module_def内のparse_port_declが呼ぶものと同一(上記に展開)
...
│  │     ├─ parse_if_stmt                          — if_stmtルールのペアを継続代入(Stmt)のリストへ脱糖する(下記補足)
│  │     │  ├─ parse_stmt_list                     — stmt_list(if/elif/elseの1本体)からStmtのリストを構築する
│  │     │  │  ├─ parse_stmt                       — 上記と同一関数
│  │     │  │  └─ parse_if_stmt (再帰)               — 入れ子のif_stmtを再帰的に脱糖する(既に脱糖済みのStmt列を連結するだけ)
│  │     │  ├─ parse_elif_clause                   — elif_clauseを(条件式, 本体)のタプルに変換する
│  │     │  │  └─ parse_stmt_list                  — 上記と同一関数
│  │     │  └─ desugar_if_chain                    — if/elif/elseの各分岐を変数ごとの三項演算子ネストへ脱糖する(下記補足)
│  │     │     ├─ branch_to_map                    — 1分岐の本体を(代入先変数名, 順序代入か, 右辺式)のリストに変換する(分岐内の二重代入はエラー)
│  │     │     └─ find_target                      — 変数名を指定してbranch_to_mapの結果から該当エントリを探す
...
│  │           ├─ is_keyword                       — 文字列がキーワードか判定する
│  │           └─ parse_ternary_expr               — 上記のparse_stmtが呼ぶものと同一の式解決チェーンへ(上記に展開)
│  └─ print_parse_result                           — Phase 1の結果(モジュール定義・テストベンチの内訳)をダンプする
...
│  │  │     ├─ resolve_module_ports                — ポート宣言を信号として登録する(clock型はoutput不可・input側は高々1つ)
│  │  │     ├─ resolve_module_decls                — 内部のvar宣言をポートと同じ信号空間に追加登録する(clock型は常にエラー)
...
│  │  ├─ elaborate_top                             — トップレベル(テストベンチの並行部分)を解決する
│  │  │  ├─ build_signals                          — テストベンチの宣言からシンボルテーブルと信号リストを構築する
│  │  │  │  └─ push_signal                         — 1つのdeclを信号として登録する(重複宣言はエラー)
│  │  │  ├─ build_instances                        — モジュールインスタンス化を走査する(1件ずつの検査・解決は下記2関数に委譲)
│  │  │  │  ├─ check_instance_name_available       — インスタンス名が信号名・既存インスタンス名と重複していないか検査する
│  │  │  │  └─ resolve_instance_connections        — 1インスタンスの引数をポート定義と突き合わせて解決する(clock型の接続も検査)
│  │  │  │     ├─ resolve_expr (再帰)                — 接続式を解決する(上記と同一関数)
│  │  │  │     └─ resolved_expr_is_clock           — 解決済み式がclock型信号への直接参照か判定する
...
│  └─ print_elaboration_result                     — Phase 2の結果(モジュール数・信号一覧)をダンプする
...
│  └─ print_netlist_result                         — Phase 3の結果(ネットリストのテキスト表現)をダンプする
│     └─ format_netlist                            — ネットリストを読みやすいテキストに整形する
└─ run_simulation_phase                            — Phase 4: initialがあればその手続きを、無ければ--cyclesでNサイクル実行する
   ├─ run_initial_sequence                         — initialが空でない場合の実行。AssignはSimulator::set_signalで即時反映し、StepでSimulator::stepを呼ぶ(表示は行わずスナップショット列を返すだけ)
   ...
   │     ├─ Simulator::should_update_reg           — regのクロックエッジ検出(clock紐付けが無ければ常にtrue)
   │     └─ mask_to_width                          — 代入結果を信号のビット幅に切り詰める(代入時のマスキング)
   └─ print_simulation_result                      — 実行結果(波形)を表示する。initial実行・--cycles実行どちらの後にも呼ばれる(下記補足)
      └─ format_waveform                           — スナップショット列をテキスト波形に整形する
```
...
- `parse_ternary_expr` は二項演算子チェーン全体（`parse_expression`）よりさらに外側にあり、`stmt` の右辺・括弧の中身のどちらもここから式の解決に入る。`?`/`:` が続かなければ条件部（`parse_expression`が返した式）をそのまま返し、続く場合は then/else をそれぞれ自分自身に再帰させることで `a ? b : c ? d : e` が `a ? b : (c ? d : e)`（右結合）になる。`parse_testbench_def`内の`parse_inst_decl`内の`parse_named_arg`・`parse_initial_block`内の`parse_proc_assign`が呼ぶ`parse_ternary_expr`は、同じ`parse_testbench_def`内の`parse_stmt`が呼ぶものと完全に同じ関数・同じ式解決チェーンなので、図では`parse_stmt`側だけ展開している。
...
- `parse_module_def`内の`parse_decl`/`parse_stmt`/`parse_if_stmt`は、`parse_testbench_def`内で呼ばれるものと全く同じ関数（`module_def`本体の並行部分も`testbench_def`の並行部分も同じ意味論のため）。図では重複を避けて`parse_testbench_def`側だけ展開している。
- `parse_if_stmt`は新しいASTノードを追加せず、既存の`Stmt::Combinational`/`Stmt::Sequential`と`Expr::Ternary`だけで`if`/`elif`/`else`を表現する（`docs/architecture.md`の「if/elif/elseの例」節を参照）。`parse_stmt_list`は本体（`(stmt | if_stmt)*`）を走査するだけの薄い関数で、入れ子の`if_stmt`は再帰的に`parse_if_stmt`を呼んで先に脱糖してしまい、その結果の`Stmt`列をそのまま連結する（つまりネストは`parse_if_stmt`の呼び出し木の中で解消され、`desugar_if_chain`が受け取る時点では既にフラットな`Vec<Stmt>`になっている）。
- `desugar_if_chain`は、各分岐（`branch_to_map`が変換した「変数名→(順序代入か, 右辺式)」のリスト）の間で代入先変数の集合と代入演算子（`=`/`<=`）が完全に一致することを検査しながら、変数ごとに`else`から`if`へ向かって`Expr::Ternary`を1段ずつ重ねていく（`if`の条件が最終的に最も外側＝最優先になるようにするため、`arms`を逆順に辿る）。不一致（一部の分岐にしかない変数、分岐によって異なる代入演算子）や同一分岐内での二重代入（`branch_to_map`が検出）はすべてパースエラーになる。
- `parse_signal_type`は`port_decl`/`decl`の両方から共通で呼ばれる同一関数（`bit`/`bit<N>`/`clock`を`SignalType`に変換する）。図では`parse_module_def`内の`parse_port_decl`側だけ展開し、`parse_testbench_def`内の`parse_decl`側は「上記と同一」と注記している。
...
- `resolve_module_ports`は`clock`型ポートを検出し、`output`なら即エラー、`input`側は2つ目が見つかった時点でエラーにする（`resolve_module_def`はその結果（`clock_port`）が`None`かつ`Sequential`文があればさらにエラーにする）。`resolve_module_decls`はモジュール本体の`clock`型`var`宣言を常にエラーにする（`clock`型の`var`は`build_signals`経由でテストベンチの`decls`からのみ許可される）。
...
- `resolved_expr_is_clock`は`ResolvedExpr::Ident(id)`かつ`signals[id].is_clock`の場合のみ`true`を返す。`clock`型は`output`ポートに使えないため`InstanceField`が`clock`型になることはなく、演算結果も常に`clock`型ではない。
- `NetlistBuilder::flatten_scope` はモジュール階層を再帰的に辿ってフラットな`signals`/`nodes`へ展開する。トップレベルスコープに対して`clock_port: None`で1回呼ばれ、`scope.instances`があればそのモジュール本体を（インスタンス名をプレフィックスにして、`module_def.clock_port`を渡して）再帰的に自分自身へ渡す。`Sequential`文の`SignalKind::Reg.clock`は、渡された`clock_port`をこのスコープの`remap`でグローバル化した`ClockTrigger`（`edge`はTODOコメント付きで`Posedge`固定）になる（`clock_port`が`None`なら`clock: None`のまま。トップレベル/テストベンチ直下のregがこれにあたる）。戻り値の`remap`/`instance_remaps`は、その直後に`build_netlist`が`initial`のAssign式を構築する際にも再利用される。展開が終わった時点で`Node`/`NetlistSignal`はモジュールの存在を一切知らないため、`Simulator::step`はどのregがどのクロックに紐付くかを`SignalKind::Reg.clock`（`Simulator::new`が`NetlistSignal`から複製した`signal_kinds`経由）で見るだけで、ノード自体の構造はモジュール対応前と変わらない。
- `Simulator::should_update_reg`は、クロック紐付けが無いreg（`Reg { clock: None, .. }`。トップレベル/テストベンチ直下のreg）またはWireなら常に`true`（既存のステップ単位更新のまま）。クロック紐付けがあれば、`Simulator::step`内で計算する`settled`（このサイクルの組み合わせ収束後の値。`snapshot`は comb 更新前なので使えない）と、1つ前のサイクルの`settled`（`prev_signal_values`）を比較し、指定エッジ（TODO: 暫定的にposedge固定）を検出したときだけ`true`を返す。
- `run_initial_sequence`は`nl.initial`が空でない場合にのみ呼ばれ、空なら従来通り`Simulator::run`（`--cycles`分だけ`Simulator::step`を繰り返す）が使われる。両者は排他的で、テストベンチに`initial`があれば`--cycles`は無視される。どちらの関数もスナップショット列を返すだけで表示は行わず、表示は共通の`print_simulation_result`に委ねている。
...
- `main.rs`の4つのフェーズ関数（`run_parse_phase`/`run_elaboration_phase`/`run_netlist_phase`/`run_simulation_phase`）は、いずれも「処理を実行する」と「結果を表示する」を分離しており、表示部分はそれぞれ対応する`print_*`関数（`print_parse_result`/`print_elaboration_result`/`print_netlist_result`/`print_simulation_result`）に切り出されている。`print_netlist_result`は`format_netlist`を、`print_simulation_result`は`format_waveform`をそれぞれ呼ぶだけの薄いラッパー。`print_simulation_result`は`run_initial_sequence`経由・`Simulator::run`経由のどちらの実行結果に対しても同じ関数が呼ばれる（図では`run_simulation_phase`の最後の子として1箇所だけ示している）。

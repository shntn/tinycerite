...
│  │  │  │  ├─ is_keyword                          — 文字列がキーワードか判定する
│  │  │  │  └─ parse_signal_type                   — block内のparse_declと同一(上記に展開)
...
│  └─ print_parse_result                           — Phase 1の結果(モジュール定義・ブロックの内訳)をダンプする
...
│  │  │  ├─ build_signals                          — 宣言からシンボルテーブルと信号リストを構築する(block側のclock型はエラー)
...
- `parse_signal_type`は`port_decl`/`decl`の両方から共通で呼ばれる同一関数（`bit`/`bit<N>`/`clock`を`SignalType`に変換する）。図では`parse_port_decl`側だけ展開し、他の`parse_decl`側は「上記と同一」と注記している。

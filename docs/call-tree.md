...
- `elaborate` はモジュール定義の解決（`build_module_defs`）→トップレベル解決（`elaborate_top`）→テストベンチの`initial`解決（`resolve_initial`）の順に進む。`resolve_module_def`と`elaborate_top`はどちらも「信号を解決 → 代入文を解決（`resolve_expr`）→ `check_multiple_drivers`/`check_combinational_loops`を適用」という同じ形をしており、`check_*`系の関数は完全に同じものを呼んでいるため、図では`resolve_module_def`側だけ展開し、`elaborate_top`側は「上記と同一関数」と注記している。`elaborate_top`は`SymbolTable`/`InstanceTable`も返し、`resolve_initial`がそれを再利用する。
